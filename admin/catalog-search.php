...
if(!isset($_SESSION['user'])){
  ...
  if (isset($_POST['search'])){
    if (!empty($_POST['search-text'])) {
      ...
      $catalog = $link->query("SELECT * FROM `catalog` WHERE `type` like '%$search%' OR `name` LIKE '%$search%' OR `color` LIKE '%$search%'");
    }
    else {
...
<body>
  ...
  <div class="catalog">
    ...
    <form action="catalog-search.php" method="POST" class="search">
      <input type="text" placeholder="Введите интересующий товар" name="search-text" >
